...
# Base scoring: higher implies stronger need for formal ownership/structure.
SCORES_2 = {"A": 2, "B": 0}
...
with st.sidebar:
    ...
    st.session_state.respondent = st.text_input("Name / Title", value=st.session_state.respondent)
    st.caption("One question at a time. Use Next/Back. Results appear at the end.")
...
st.write(f"Progress: {min(step, total)}/{total}")
...
if step >= total:
    ...
    with c4:
        if st.button("Edit answers"):
            st.session_state.step = max(total - 1, 0)
            st.rerun()

else:
    # Render current question
    qid, qtext, options, score_map, weights = QUESTIONS[step]

    st.subheader(f"Question {qid}")
    st.write(qtext)

    prev = st.session_state.responses.get(qid, None)
    choice = st.radio(
        "Select one:",
        options=list(options.keys()),
        index=(list(options.keys()).index(prev) if prev in options else 0),
    ...
    with b3:
        st.caption("Use Next/Back. Results appear after the last question.")
